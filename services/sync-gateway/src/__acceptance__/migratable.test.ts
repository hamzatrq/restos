...
/**
 * The seventeen tables `plans/wave-1/running-the-stack.md` §2 tells an operator to expect. Written
 ...
 * artifact key; 11-F23 — the device-plane PIN credential in its own table), and `config_entries` /
 * `config_versions` at `0013` (01-F87 — `00 §7`'s layer-2 configuration as the FOURTH `01-F75`
 * reference-data resource, org-scoped). This list is a FACT about the shipped migrations, so it
 * moves with them; the assertions using it are unchanged and are still exact equality, which is why
 * adding a table cannot pass here unnoticed. ⚠ *It read "fifteen" and held fifteen entries until
 * `0013` landed; the WORD and the LIST move in one edit, because a count in prose beside a
 * `grep`-able fact is the staleness this file's own §T1 note is about.*
 */
...
describe("services/sync-gateway migrations are runnable as a deploy step (the seam to a deploy)", () => {
  ...
  it("RESUMES a torn schema: a rolled-back tail is re-applied, not wedged", async () => {
    ...
    // assertion. `1785000900000` is `0013_config_plane`'s `when`; it was `1785000800000`
    ...
    try {
      await sql`delete from drizzle.__drizzle_migrations where created_at = 1785000900000`;
      // `0013` is two CREATE TABLEs and one CREATE INDEX and **NO ALTER and no backfill** —
      ...
      //
      // ⚠ **THE PREVIOUS TEAR-OFF HAD AN ALTER AND THIS ONE DOES NOT, WHICH IS THE HALF THAT GOES
      // WRONG BY COPYING.** `0012` also relaxed `users.email` to nullable, so its tear-off had to
      // put the constraint back or the re-apply would meet a schema the migration did not expect;
      // a session that copied that line into this block would be undoing a change `0013` never
      // made. Read the migration, not the last tear-off.
    ...
    // off the whole tail would look identical to tearing off the last one. `staff_entries` is
    // `0012`'s, i.e. the migration IMMEDIATELY below, which is the sharpest witness available:
    // `orgs` (`0010`) would survive tearing off two.
