...
- **Cloud uplink is per-device (DEC-SYNC-004, accepted):** every device runs its own cloud session — the hub does NOT proxy peers' events to the cloud (the wire `push` carries own events only; no on-behalf semantics exist). The hub relays on the LAN; each device syncs to cloud independently. Hub-proxied single-branch uplink is a deferred protocol addition for constrained-WAN branches.
