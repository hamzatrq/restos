...
export type KotLine = {
  ...
  /**
   * `03-F56` — `02-F6`'s item note, *"printed prominently on the KOT"*, and `03-F3`'s *"item
   ...
   *
   * Optional because most lines have none and `03-F56` gives an absent note no row. The named
  ...
  KOT_ITEMS: (data) =>
    kotOf(data).lines.flatMap((line, index): readonly EncoderPart[] => {
      ...
      return [
        ...
        /**
         * `03-F56` — the note is the LAST row of its own item block, after that item's modifiers,
