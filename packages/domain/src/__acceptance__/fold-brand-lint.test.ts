 ...
 * startup and machine load, not by the assertion — the identical harness `money-rupee-lint`
 * documents, for the identical reason.
 *
 * That file was given this budget in `ff7b750`, which named the flake correctly and fixed ONE of
 * the THREE files carrying the harness. This file kept vitest's 5 s default and went on failing:
 * a full `pnpm test --force` measured this leg at **5310 ms** against the 5 s default and went
 * red, on an unchanged tree whose previous run was green. Three consecutive clean runs is exactly
 * the sample size that misses this.
 *
...
describe("T-01-22 lint gate — the money-arithmetic ban reaches the FOLD plane (18 §4 / DEC-MONEY-005)", () => {
  ...
  it(
    "18 §4 / DEC-MONEY-005 (fold clause): BINARY arithmetic on a money MEMBER expression fails the lint gate",
    () => {
  ...
  it(
    "18 §4 / DEC-MONEY-005 (fold clause): the shipped `billedCellPaisa` idiom — qty × unit_price on a member expression — fails the lint gate",
    () => {
  ...
  it(
    "18 §4 / DEC-MONEY-005 (fold clause): COMPOUND assignment whose RHS is a money MEMBER expression fails the lint gate — this is the shipped accumulator idiom",
    () => {
  ...
  it(
    "18 §4 / DEC-MONEY-005 (fold clause): a money member expression read out of an `unknown`-typed payload map still fails the lint gate",
    () => {
  ...
  it(
    "18 §4 (GREEN, must stay green): arithmetic on NON-money member expressions lints clean — the extension stays scoped to money names",
    () => {
      const { exitCode, output } = lintSource(
        src(
  ...
  it(
    "18 §4 (GREEN, must stay green): COMPARISON and plain assignment on a money member lint clean — only arithmetic is banned",
    () => {
  ...
  it(
    "18 §4 / DEC-MONEY-005 (GREEN, must stay green): money members routed THROUGH the domain helpers lint clean — the ban must never flag the blessed path",
    () => {
      const { exitCode, output } = lintSource(
        src(
  ...
  it(
    "18 §4 (GREEN, must stay green): money-named STRING keys and ids lint clean — only VALUES are money, keys are not",
    () => {
