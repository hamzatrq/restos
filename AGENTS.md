...
3. **The canonical ordering key is changing, and device clocks are not trustworthy.** `spec 25 §11–§16` recommends replacing `(global_seq ?? +∞, device_created_at, …)` with a **clock-free causal order** `(causal_seq, device_id)`, demoting `global_seq` to a delivery cursor (`DEC-PERF-001`, awaiting ratify). Until then: **do not add new reads of `device_created_at`**, and never treat it as true time — it is a raw untrusted clock read. Anything needing real time belongs to the time layer (`DEC-TIME-001`), not the ordering key. `lamport_seq` is a per-device **gap-free** counter for transport/audit — it is *not* a causal clock and must not be repurposed as one.
