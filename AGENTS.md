...
**Repo state (update this line when it changes):** Wave 0 kernel merged; Wave 1 work is on **`feat/wave-1-ui-foundation`** (71 commits ahead of `main`, not yet pushed). Suites **GREEN**: **1274 tests** (sync-client 448, gateway 247 on Testcontainers, ui 213, domain 171, sync-protocol 77, testing 49, pos-electron 36, escpos 33); `pnpm verify` exit 0. **Run `pnpm test --force --continue`** — `--force` because a cached turbo run has produced a false green here, and `--continue` because **turbo KILLS a failing task's siblings**, so without it seven green suites are indistinguishable from seven killed ones (that hid 3 real `ui` failures for an hour). **Do not trust ONE green run:** four subprocess-spawning oracle suites have flaked on a 5 s default across two packages (`biome` via `spawnSync`, `tsc` via `execFileSync` — search the PROPERTY, not the mechanism), and a killed suite leaves `.oracle-typecheck/` in the tree which then reds `pnpm verify` on generated fixtures. **THE COUNTER LOOP RUNS END TO END:** `C4` start an order (no default order type, founder ruling) → `C5` tap a tile for a priced line → `C9` send to kitchen → settle (`02-F12/F13`). **The `24 §3` test-authorship split is now REAL, not aspirational** — `T-2`, `T-3` and `K-1` were each authored by a separate session from spec text, committed RED, then implemented against; briefs live in `plans/wave-1/t2-t3-test-brief.md` and `plans/wave-1/kot-test-brief.md`, and their **⚠ Do NOT read** sections are load-bearing. **Pricing is per (branch, channel)** (`01-F60`, founder ruling: commission drives it) with **no fallback** — completeness is enforced at the writer, and the editor prefills (`14-F29`). `02-F42` closed `channel` (it is a PRICE KEY now); `03-F50` put `station` on the catalog entry, inheriting down the `01-F21` chain. **Wave 1's foundation:** `specs/27` (67 FRs incl. §2b thermal paper), `plans/wave-1/role-task-inventories.md`, `plans/wave-1/screen-map.md`, `specs/03-F30..F50`. **`packages/ui`:** 15 components, theme-aware, with a happy-dom suite (`.dom.test.tsx`). **`apps/pos-electron` RUNS** — `pnpm rebuild:native` once, then `pnpm start`. **⚠ READ `plans/wave-1/oracle-round-2-findings.md` §C before writing any test here** — its three test-authorship failure patterns have each recurred *inside the work that was fixing them*, twice more this round (a tripwire that stayed vacuous after its blocker cleared; a no-degradation guard that banned a field it required). **Owed on Wave 1, in priority order:** (1) **KOT printing** — `plans/wave-1/kot-printing.md` APPROVED, K-1 landed, K-2/K-3/K-4/K-5 need test sessions; **no printer yet**, so the physical pass is owed and no test may imply otherwise. (2) **the back office** — `plans/wave-1/backoffice-catalog.md`, BLOCKED: Commandment 8 names a `domain` permission matrix that **has never been written**, and there is **no user authentication anywhere in the corpus** (`01-F47` covers devices, not people). (3) the rest of the service surface — shifts, cash/day, aging timers, alarms, approvals. **Open questions, all recorded in their plans:** `modifier` is unclassified by `01-F60`; `DEFAULT_STATION`'s value is pinned not specified; `publishCatalog`'s enabled-set is caller-supplied so a forgetful caller gets NO completeness check; `03 §7`'s derivation cannot reproduce its own 44/42/48 at 576 dots; `02-F1` writes "WhatsApp" where `02-F42` writes `whatsapp`. **Remaining Wave 0:** H-01 harness rungs + physical wall-clock (D3). `pnpm verify` = docs-lint + typecheck + lint; `pnpm test` = the suites (Docker required).
