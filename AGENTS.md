...
**Repo state (update this line when it changes):** Wave 0 kernel **MERGED to main** (`c91a064`). Fourteen tasks through the full `24 §3` loop; suites green (domain 137, sync-protocol 62, sync-client 320, testing 49, gateway 195 on Testcontainers, ui 23). **Wave 1 is UI-first by founder ruling** and is in its foundation phase: four parallel research passes → **`specs/27`** the design language (62 FRs incl. **§2b thermal paper**, which discharges the gap 27-F11h named as Wave 1's hardest problem) · **`plans/wave-1/role-task-inventories.md`** (114 tasks, 13 roles, 224 verified FR IDs) with **all ten conflicts now CLOSED** · **`plans/wave-1/screen-map.md`** the shell + IA derived from those tasks · **`specs/03-F30..F47`** the printed-document model (`DocumentSpec`/`DocumentProfile`) plus the three device failure modes that would otherwise have shipped · **`packages/ui`** the token layer with `tokens.test.ts` re-deriving doc 27's arithmetic on every commit, five components, and Storybook 10 building clean. The `01 §4` **event-catalog absorption debt is cleared and now machine-enforced** (docs-lint C6). **Remaining Wave 0:** the H-01 harness rungs + physical wall-clock (p95/plug-pull, D3 — the kernel has never run on target hardware); the filed follow-ups in `plans/wave-0/`. **`packages/ui` is built:** 12 components (the full `screen-map.md` §6 vocabulary + `AppShell`), 13 story files incl. the composed counter screen and 22" pass panel, **43 tests**, Storybook 10 green. An **adversarial review round found 5 real defects** (opacity washes putting disabled-reason text at 1.97:1 and defeating `27-F4`; a destructive control below the 48 dp floor; `bgColor-` tokens used as foregrounds; a comment overstating its code; and my own fix for the third, caught by the new guard) — all fixed, and `components/discipline.test.ts` now enforces each structurally. **Training mode is answered** — `DEC-TRAIN-001`/`01-F49..F51`: it is a *branch*, not a flag, so the kernel gains nothing. Six inventory gaps closed (`02-F37..F39`, `03-F48`, `27-F63`). **`apps/pos-electron` started:** `18 §9` process split, a typed IPC contract whose laws are what it *cannot* express (no query channel, no mutation channel), main-process gateway + 13 tests. Building it against the real kernel surfaced three findings — **`plans/wave-1/pos-shell-findings.md`; F1 is the biggest remaining Wave-1 gap: there is NO CATALOG on the device, so no screen can render an item name or price.** **Owed on Wave 1:** an **independent oracle pass over `packages/ui` and `rupeesFromPaisa`** (implementation and tests were written in one session, which `24 §3` step 2 forbids; `domain` is a protected path); the ~11 remaining §10.1 gaps, each needing a founder product call not a spec edit; `DEC-TRAIN-001` founder ratification. `pnpm verify` = docs-lint + typecheck + lint; `pnpm test` = the suites (Docker required).
