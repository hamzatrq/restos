...
**Repo state (update this line when it changes):** Wave 0 kernel merged; Wave 1 work is on **`feat/wave-1-ui-foundation`** (54 commits ahead of `main`, not yet pushed — `main` sits one ahead of `origin/main`). Suites **GREEN**: **1170 tests** (sync-client 429, gateway 231 on Testcontainers, ui 213, domain 162, sync-protocol 62, testing 49, pos-electron 24); `pnpm verify` exit 0, `pnpm test --force` exit 0 over **four consecutive runs**. **Always `--force`, and do not trust ONE green run** — a cached turbo run has produced a false green here before, and in July 2026 a single run was green while the next failed on an unchanged tree. **Two evidence order-dependencies were found and fixed; know they existed:** three files spawn Biome per case and only one had the 60 s budget, so `fold-brand-lint` flaked at 5310 ms against vitest's 5 s default (the same leg measures 951–1905 ms unloaded); and a killed `packages/ui` task leaves `.oracle-typecheck/` in the tree, which made `pnpm verify` red on generated fixtures — now gitignored. **Wave 1 is UI-first by founder ruling.** Its foundation is **complete**: four research passes → **`specs/27`** the design language (67 FRs incl. **§2b thermal paper**) · **`plans/wave-1/role-task-inventories.md`** (114 tasks, 13 roles) with all ten §9 conflicts CLOSED and all seventeen §10.1 gaps dispositioned (**G9** deferred to Wave 4) · **`plans/wave-1/screen-map.md`** · **`specs/03-F30..F48`** the printed-document model. **`packages/ui`:** 15 components incl. `Surface` (27-F43) and `TenderPanel` (02-F12/F13), theme-aware via `ThemeProvider`/`useColor` — which is how **27-F19**'s KDS dark opt-in and **27-F67**'s training inversion both work — and it now **has a DOM**: a `.dom.test.tsx` suffix opts a file into happy-dom, so behaviour is observed rather than grepped for. **The catalog transport is built AND USED** (`plans/wave-1/catalog-transport.md`): a device fetches its catalog over the wire on `hello_ack` version mismatch. **`apps/pos-electron` RUNS** — `pnpm rebuild:native` once, then `pnpm start`. **The oracle round-2 findings are ALL CLOSED** (`plans/wave-1/oracle-round-2-findings.md` — still read it before touching any of the above; six reviewers found ~40 defects incl. a remote-origin path to the ledger). **Owed on Wave 1, in priority order:** (1) **the counter cannot ADD a line** — `Counter.tsx`'s `onSelect` is a no-op and is *unbuildable*, not unbuilt: `order.line_added` needs `unit_price_paisa` and no layer of the catalog path carried a price. Specced July 2026 (**`01-F60`** per-(branch,channel) catalog pricing, **`02-F42`** `channel` closed and a price key, **`14-F29`** the prefilled editor); T-2..T-5 in **`plans/wave-1/channel-pricing-and-the-counter-loop.md`** build it, and **T-2/T-3 are protected paths whose acceptance tests a SEPARATE session authors first** — brief in `plans/wave-1/t2-t3-test-brief.md`. (2) Wave 1's remaining service surface — shifts, aging timers, alarms, approvals, owner summary. (3) KOT printing on confirm (doc 03, needs hardware). **Open, routed to doc 15:** the gateway has no org/branch registry, so `publishCatalog` takes the branch set from its caller. **Remaining Wave 0:** the H-01 harness rungs + physical wall-clock (D3 — the kernel has never run on target hardware); follow-ups in `plans/wave-0/`. `pnpm verify` = docs-lint + typecheck + lint; `pnpm test` = the suites (Docker required).
