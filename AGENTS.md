...
**Repo state (update this line when it changes):** Wave 0 kernel **COMPLETE** on `feat/t-01-16-transport` (51 commits ahead of main, pushed; awaiting CODEOWNERS senior review — every task touches protected paths). Eight tasks each through the full `24 §3` loop (oracle→implement→adversarial-review→fix): **T-01-15** merge-semantics fold engine (replaced the O(N²) comparator; zero-fold-work adoption), **T-01-12** hub-relayed cloud uplink (DEC-SYNC-009 launch blocker), **T-01-13** money helpers + GritQL raw-arithmetic ban (DEC-MONEY-005), **T-01-08** quarantine pipeline (DEC-SYNC-005/007/008), **T-01-09** device auth (jose + registry + revocation, DEC-SYNC-009 origin-existence), **T-01-11** Auditor v1 (five READ-ONLY legs, 20 §4.2), **T-01-16** batched catch-up + zstd. `verify:01` = **34 green / 0 red / 13 unmapped of 47**; full suites green (domain 98, sync-protocol 32, sync-client 264, testing 49, gateway 151 on Testcontainers). Filed follow-up docket in `plans/wave-0/t-01-*-fix-round.md`. **Remaining Wave 0:** the H-01 harness rungs + physical wall-clock (p95/plug-pull, D3); the filed MED/LOW follow-ups; DEC-TIME-001 (time layer) still open; the three `DECISION PENDING` product constants still founder-owned. `pnpm verify` = docs-lint + typecheck + lint; `pnpm test` = the suites (Docker required).
...
3. **The ordering design is RATIFIED: per-fold merge semantics (`26`, `DEC-PERF-001` accepted).** `01-F34` is rewritten — folds declare explicit merge rules and read **no ordering metadata** (no `global_seq`, no `lamport_seq`, no device clock; tested by bijective id-relabel + injection invariance — plain convergence is insufficient, min-id passes it wrongly). `global_seq` = delivery cursor + compaction watermark only. The shipped comparator engine in `sync-client` implements the SUPERSEDED law until **T-01-15** lands. `causal_seq` remains refuted. Still no new reads of `device_created_at` (`DEC-TIME-001` open); `lamport_seq` keeps its gap-free transport/audit role.
