...
**Repo state (update this line when it changes):** Wave 0 kernel merged; Wave 1 work is on **`feat/wave-1-ui-foundation`** (42 commits ahead of `main`, not yet pushed — `main` sits one ahead of `origin/main`). Suites **GREEN**: **1130 tests** (sync-client 425, gateway 219 on Testcontainers, ui 193, domain 162, sync-protocol 62, testing 49, pos-electron 20); `pnpm verify` exit 0, `pnpm test --force` exit 0. **Always `--force`** — a cached turbo run has produced a false green in this repo before, and it cost a wrong claim in a commit message. **Wave 1 is UI-first by founder ruling.** Its foundation is **complete**: four research passes → **`specs/27`** the design language (67 FRs incl. **§2b thermal paper**) · **`plans/wave-1/role-task-inventories.md`** (114 tasks, 13 roles) with all ten §9 conflicts CLOSED and all seventeen §10.1 gaps dispositioned (**G9** deferred to Wave 4) · **`plans/wave-1/screen-map.md`** · **`specs/03-F30..F48`** the printed-document model. **`packages/ui`:** 14 components incl. `Surface` (27-F43), theme-aware via `ThemeProvider`/`useColor` — which is how **27-F19**'s KDS dark opt-in and **27-F67**'s training inversion both work. **Doc 27 gained two FRs from a founder ruling:** `27-F66` (neutral surfaces carry an OUTLINE, because SC 1.4.11 on all three is *provably* incompatible with 27-F21 — 14,196,198 qualifying triples, zero admitting any text colour) and `27-F67` (training renders the OPPOSITE POLARITY, 14.31:1, because no tint can exceed 1.08:1 and keep AA). **The catalog transport is built** (`plans/wave-1/catalog-transport.md`, APPROVED): wire frames, gateway publish/serve, device fetch on `hello_ack` version mismatch. **`apps/pos-electron` RUNS** — `pnpm rebuild:native` once, then `pnpm start`; a real counter screen renders from the device catalog. **⚠ READ `plans/wave-1/oracle-round-2-findings.md` BEFORE TOUCHING ANY OF THE ABOVE.** Six independent reviewers found ~40 defects in the nine commits that built it, including a path by which a remote origin could append forged events to the ledger (fixed). **Owed on Wave 1, in priority order:** (1) the **round-2 fix list** — `publishCatalog` validates nothing and one blank name is an org-wide sync outage; a publish mid-snapshot splices two menus; a `catalog_notice` mid-fetch commits half a menu permanently; the AST discipline scanner goes blind after a file's first template literal; nothing binds a component to `useColor()`; two status fills render no outline at 2.35–2.91:1; `sec-F1` resolves to nothing. (2) **nothing constructs a cloud session** — the transport is real inside `sync-client` and no app calls it, so `catalog_request` has never left a device and the app's menu comes from a local store only. (3) **`01-F59` needs a spec answer**: it says an 86'd item stays deliberately sellable and `Tile` renders it `disabled`. (4) Wave 1's service surface is unbuilt — payments, shifts, aging timers, alarms, approvals, owner summary. **Remaining Wave 0:** the H-01 harness rungs + physical wall-clock (D3 — the kernel has never run on target hardware); follow-ups in `plans/wave-0/`. `pnpm verify` = docs-lint + typecheck + lint; `pnpm test` = the suites (Docker required).
