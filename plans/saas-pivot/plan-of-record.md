...
| R78 | Customer PII before envelope encryption | **RECORD NOW, AND THE UN-ERASABILITY IS ACCEPTED.** `22-F26` (b) names this lever and declines to pull it, calling it *"a product call about phone and delivery orders, and not doc 22's to take"* — this is that call. **Cost accepted, in full:** the ledger is append-only, so every phone, name and delivery address written before `22-F14`'s envelope encryption ships is **permanently un-erasable** (`01-F1`), and the fallback is field-level redaction with hash-chain carve-outs — the alternative `DEC-DATA-001` exists to avoid. **Loyalty is the feature whose entire purpose is to grow that number.** The alternative was refused because holding takes **four shipped surfaces** dark — loyalty, khata, delivery address, repeat-caller lookup — for a wrapper that is `§9.3`'s uncosted spike with no date. ⚠ **Defensible for a handful of pilots and NOT defensible at scale**: the wrapper's cost should be established before the tenant count makes redaction the only route. |
| R79 | Loyalty forms | **THREE, NOT TWO: account loyalty, a physical bearer card, and a Google/Apple WALLET PASS.** `17-F15` bans punch cards **by name** and the founder asked for them by name; the ban is overruled and a spec act is owed to doc 17. Founder's words: *"one is just a special discount type if some restaurants want to really use card stamping, some might prefer our system. but there is another google/apple wallet based system. we want that as well."* **The design's reading survives and is what makes this cheap:** account loyalty and a bearer card are different objects with different fraud models, differing in one field — so the card is a **campaign kind**, not a second mechanism. ⚠ **THE WALLET PASS IS NEW SCOPE AND IS NOT IN THE CORPUS ANYWHERE** — it is a third bearer form with real platform work behind it: Apple Wallet requires a pass-type identifier and a signing certificate from Apple, Google Wallet an issuer account and a signed JWT, and both want a push path so a stamp appears without the customer reopening anything. **It is bearer-shaped like the physical card**, which is why it fits the same campaign kind rather than a fourth model — but the *issuance and update* half is a genuine integration and must be costed, not assumed. |
| R80 | May the AI layer write to the ledger? | **NEVER. Everything is computed on read.** Ships two slices sooner at zero kernel risk. ⚠ **The price is real and accepted:** an alert **re-nags for ever** because nothing can acknowledge it, `12-F17` stays unbuildable, and last month's brief **silently changes when a fold changes** — a history that is not stable is a history an owner cannot cite. The alternative buys dedupe, cross-surface ack, an alert inbox and stable history, and costs a doc-01 amendment on the most expensive protected path in the corpus — **and the door argument decided it: once the cloud emits one event family, the next module asks for its own.** Revisit after the pilot, when re-nagging and drifting history have either hurt or not. |
| R81 | Build sequencing | **ALL FIVE MODULES IN PARALLEL, EACH IN ITS OWN WORKTREE.** ⚠ **CRM is the shared dependency and is not optional ordering** — loyalty, storefront and messaging all need the order→customer link, and `shared/ipc.ts:768` already names the hole: *"no event in the corpus can say which customer an order is for"*, blocking **four** features on **one** field. So CRM leads by a nose and the rest follow immediately rather than waiting. **Cost accepted:** five review queues, and merges that will conflict where modules touch the same surfaces — the counter especially, which every one of them wants. ⚠ **The AI layer is the exception and the reason is data, not sequencing:** its own plan measures that most of what it would analyse does not exist yet — no inventory, no customer history, thin reporting — so it is planned and held rather than built alongside. |
