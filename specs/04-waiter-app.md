...
- 04-F27 **THE TERMINAL'S WRITE PATH IS THE TILL'S WRITE PATH — every guard, and every consequence.** `04-F21` makes the pad a remote renderer of the till, and build 1 delivered that for the ENVELOPE (one identity, one device id, one price resolution) and not for the road the envelope travels. Two halves, both measured on a real store:
  ...
  - **Both halves are invisible to a reading and to every suite that drives one path**; what finds them is running the other path over the same store, and that is the shape of the assertions this FR requires.
