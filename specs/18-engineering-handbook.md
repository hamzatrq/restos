...
- Text prints via printer fonts (English + numerals — sufficient under the English-only decision, 00 §5.6). **The "bitmap raster path is for logos and QR only" clause is WITHDRAWN (July 2026)** — it contradicted `03-F8` and `00 §5.6`, and it was factually wrong: **the second script is not hypothetical, it is in every customer name and address the system already stores today.** Non-Latin user content rasterises per `03-F8`. Two independent hardware facts force this and neither is a preference: character-mode ESC/POS performs **no contextual shaping** (the ROM font renders each byte as an isolated glyph — no joins, no ligatures) and **no bidi reordering** (glyphs emit left-to-right in byte order), so Urdu on a character-mode printer is architecturally impossible, not merely hard. Both major ESC/POS libraries independently recommend rasterising such text, and one reports Arabic-Indic numerals rendering as placeholder characters on real hardware — incidental corroboration of `27-F22`'s Western-digit rule. **Open, and not to be specified before it is measured (`00 §4` rig):** whether whole-document rasterisation fits `03-N1`'s 2 s confirm→first-byte budget. Per-FIELD rasterisation is the tempting answer and breaks the `name | price` column grid — the one row whose alignment carries the meaning.
...
- **Core/shared:** `typescript`, `zod`, `uuidv7`, `date-fns` + `@date-fns/tz`, `superjson`, `tsx` (dev), `@biomejs/biome`, `turbo`, `@noble/hashes` (audit hash-chain — pure-JS/sync/cross-runtime SHA-256; DEC-AUDIT-001), Node built-in `node:zlib` zstd (compressed catch-up wire framing — sync, no new npm dependency per §15 rule 1; T-01-16, 01 §5, 26 §6.4, DEC-SYNC-010 candidate)
- **Backend:** `fastify`, `@fastify/websocket|cors|rate-limit`, `@trpc/server`, `drizzle-orm`, `drizzle-kit`, `postgres` (driver), `bullmq`, `ioredis`, `pino`, `pino-pretty` (dev), `jose`, `argon2`, `@anthropic-ai/sdk`, `@sentry/node`, OpenTelemetry SDK packages, `ws`, `@peculiar/x509` (pinned `1.x` — X.509 ISSUANCE for `01-F73`'s LAN device credentials; Node parses certificates and does not construct them, and hand-rolled ASN.1 DER is security-critical encoding well past §15 rule 1's "50 lines of our own code" bias. Pinned below `2.x` — ⚠ **and the first reason recorded here was FALSE, corrected August 2026 by the `20 §4.4` review lane.** It said `2.x` *"pulls `tsyringe` + `reflect-metadata`"* as though `1.x` did not; `1.12.3`'s own `dependencies` carry both, and the lockfile resolves them. The pin buys nothing on dependency count. **The real, reproduced reason:** `2.0.0`'s CommonJS entry throws `tsyringe requires a reflect polyfill` on import unless the consumer imports `reflect-metadata` first, so adopting it means a side-effect import in every entry point that touches PKI — a thing that is silently correct until someone adds a new one. `1.12.3` imports clean. *A justification is a claim, and this one was a guess dressed as a measurement; the correction is kept rather than overwritten because §15 step 3 makes the transitive tree the reviewer's job and this is what it looks like when nobody does it.* Lives in `packages/lan-pki`, reachable from the cloud plane and test support only; it must never enter `packages/sync-client`, because a device generates its keypair with `node:crypto` and receives a finished certificate)
