...
| DEC-SYNC-005 | Cloud ack watermark advances over quarantined events | proposed | 01-F37 / 01-F8 | A quarantined event is durably stored, so the push ack advances past it — the device outbox never wedges on a poisoned event (01-F17 spirit). Consequence: Auditor lamport-gap checks must count quarantine rows as filling their slots (T-01-11). Needs a 01-F37 clarifying sentence on promotion | Founder + 01 spec sentence | W0 exit |
| DEC-SYNC-006 | LAN heal set-union when the origin departed | proposed | 01-F38 / HUB-ELECTION.md | Followers push own events only; a departed origin's events held by a demoted hub cannot cross a healed partition on LAN — set-union law is cloud-catchup-dependent in that shape. Options: follower third-party relay (protocol addition) vs accept cloud dependency (document in HUB-ELECTION.md). T-01-06 must exercise the scenario | Founder + protocol call | W0 exit |
