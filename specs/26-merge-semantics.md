...
> **⚠ AMENDED 2026-08-23 — FIVE OF THOSE EIGHT NOW HAVE SCHEMAS, AND THE CONCLUSION SURVIVES FOR A DIFFERENT REASON.** Measured against the 34 payload schemas declared in `packages/domain/src/registry.ts`, comment-blind: **still schema-less** are `table.state_changed`, `order.merged` and `payment.split_recorded`; **shipping a schema** are `availability.changed`, `shift.opened`, `shift.closed`, `void.recorded` (`registry.ts:673`), `comp.recorded` (`:682`), `discount.recorded` (`:688`), and — where this sentence said `cash.*` as a blanket — **all three** cash types the corpus declares, `cash.paid_out`, `cash.deposit_recorded` and `cash.drawer_opened`. ⚠ *The first draft of this very amendment wrote a fourth, still-absent member named cash-dot-counted (written here without backticks on purpose, so this sentence does not itself trip the rail). No such event type exists: `docs-lint` C2 refused the file by name, which is commandment 2 caught by a rail inside the act correcting a different staleness — the failure mode this corpus records as* a stale claim propagates fastest through the person fixing a different stale claim in the same paragraph.
> 
> **The trailing clause is still TRUE and its REASON has changed, which is the part that matters.** `void_value`, `comp_value` and `discounts` still evaluate to zero — not because the schemas are missing but because **nothing in the product emits them**: measured the same day, all three have a schema, an inert fold arm (`packages/sync-client/src/folds/merge.ts:866`–`868`, which the file itself calls *projection-inert … a stated DEBT*) and **zero production emitters**, because the counter has no void, comp or discount surface at all. `DEC-MONEY-010`'s gate names three conditions and `01-F83` supplies only (ii); (i) the emitters and (iii) this document's own oracle-pinned merge rule are both still owed.
> 
> **⚠ THIS LINE WAS LOAD-BEARING IN SHIPPING CODE AND THAT IS WHY IT IS CORRECTED RATHER THAN QUIETLY EDITED.** Four files inherited the retired premise: `services/api/src/summary.ts`'s `OMISSIONS` table, which is **rendered to an owner** in the nightly summary and states as fact that these types have *no payload schema in `packages/domain`*; and `packages/escpos/src/receipt-document.ts`, `cash-documents.ts` and `min-columns.ts`, the last of which **derives a numeric column floor** (`day_summary: 34`) from a line justified by this sentence. The number is unaffected — it comes from the label `Voids/comps/discounts NOT RECORDED`, which stays correct while there is no emitter — but a figure whose stated derivation cites a retired premise is a figure the next reader cannot check. **A spec sentence quoted into shipping code acquires the code's shelf life, not the spec's**, and this corpus has recorded the same shape twice before: a green test defending an overruled rule, and a shipped comment promising a rail protection that did not exist. `packages/escpos` is a protected path (commandment 10), so those three comments are **reported here, not edited from a spec act**.
...
**RATIFIED (founder, July 2026).** Merge semantics is the selected design; `DEC-PERF-001` is accepted and promoted — `01-F34` is rewritten, `01-F29/F30/F31/F32/F33` amended, `order.settlement_closed` added to the `01 §4` catalog. The schema additions of §7, the measured `04-F17` privacy trade, and hub-as-business-emitter are ratified. Implementing task: **T-01-15** (fold engine + domain schemas). **Both remaining openings are now closed (founder, July 2026).** `DEC-TIME-001` is **accepted** and promoted into `01-F43..F46` — `confirmed_at`/`kot_at`/`age_basis` stop reading `device_created_at` and take branch-consensus time; implementing task **T-01-17**. The four §5-class product constants are **ratified at their matrix defaults**, so `packages/domain/src/product-constants.ts` carries ratified law rather than `DECISION PENDING` analysis defaults: contested lines remain billable (§5.4 — an unapproved void must not silently erase revenue, 02-F20); `false` wins the availability race (§5.8 — the conservative read among concurrent heads); a two-head KOT prints the explicit conflict marker (§5.10 — there is no clock-free "default head" to print, and paper structurally requires one string); and excess tender is **not** an exception (§5.3 — with tips unmodeled (DEC-MONEY-004) and no cash-rounding rule in the corpus, firing the shift-close variance alarm on "keep the change" would alarm on the base case of a Friday night, against the 01-F17 never-block spirit). Each remains one named constant with one place to overrule it. `DEC-PERF-001` remains open, `causal_seq` is not to be implemented, and **T-01-14 is paused** — not because its work is wasted (the projection-key sidecar is needed under every candidate) but because it was scoped to an *entity* index and a back-filled workaround for a trap that a one-field schema fix removes.
