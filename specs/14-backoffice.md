...
- 14-F36 **Publish timing belongs to the ACT, not to the entry.** `14-F28` is not re-opened: the choice stays per edit, the default stays the 05:00 boundary, apply-now stays explicit with its consequence on the control. What this FR decides is **where the question is asked**.
