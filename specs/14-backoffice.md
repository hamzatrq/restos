...
6. Whether an owner assigned to a single branch should be able to revoke a device at that branch: 14-F30's cell is `owner: allow`, but neither device procedure states a `branch_id` (it is read from the registry *inside* the revocation, so a caller-stated branch would be checked after the destructive act), so the effective permission today is **org-wide owner only** — a question about the customer model (does a multi-branch org ever scope an owner?), not about this FR's cell.
