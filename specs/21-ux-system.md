...
- 21-F15 **THE NAMING LAW (August 2026): no surface renders a machine identifier where the product knows a name — and where it knows none, that is a MISSING FIELD, not a rendering problem.** This is layer 1's argument (constrain what can be generated) applied to data rather than components, and it exists because the failure is invisible to every layer above it: a screen showing `a3f1c7e2-…` where a branch name belongs passes typecheck, passes its story, passes its `.dom.test.tsx` assertion that the label is present, and is nonsensical to the only person who matters — which is precisely the class this document's opening paragraph says nothing so far verifies.
  - **Definitions, so the rule is checkable rather than a matter of taste.** A **machine identifier** is any value whose only purpose is to be a key: org, branch, device and user ids, event and entity ids, session and attempt keys. A **name slot** is any place a surface presents such an entity to a human — a heading, a list row, a tile label, a report axis, a printed document field, a spoken or logged sentence a staff member reads.
  - **The rule:** a name slot renders the record's name (`01-F68` org, `01-F69` branch, `01-F70` device, `11-F20` person). Where the record has no name, the slot renders a stated **unnamed** treatment that says what is missing and where it is set (00 §5.7 honesty). It never renders the identifier as though it were the name, and it is never blank — a blank is the same lie with less information.
  - **The counterpart, which is the half that changes what gets built.** An unnamed record is a **missing field upstream**, so the fix is the required field on the record, never a prettier fallback. Every *unnamed* treatment site is therefore **enumerated**: the count is a visible debt that shrinks, and adding one is a deliberate act with a reason. A law satisfied by rendering the word "Unknown" everywhere would be worse than no law, because it retires the question.
  - **Exceptions, enumerated and narrow.** (a) Support and diagnostic surfaces where the identifier *is* the subject — doc 15's fleet health (15-F11), quarantine views and diagnostics bundles (15-F18). (b) A secondary, explicitly labelled **technical id** offered for support beside a name, copyable. A technical id may never occupy the name slot, and neither exception reaches a staff-facing operational surface or a customer-facing one.
  - **How it is checked — mechanically, and not by grep.** The rail is an extension of the existing renderer gate that already mounts shipped surfaces in a real browser and measures them: it additionally sweeps visible text nodes, and a node whose text matches the identifier shape and is not inside a component declared a technical-id slot is a violation, reported by surface and control. ⚠ **A grep over the per-app `strings.ts` catalogs (00 §5.6) is NOT this check and must not be substituted for it** — the defect is a runtime interpolation, never a string literal, so a catalog sweep is green on every real instance of it.
  - **Stated limits, because a rail oversold is a rail trusted wrongly.** It sees only the states its fixture produces — the same boundary the layout gate has measured on itself — so a name slot reachable only in a state nobody fixtures is unmeasured. And it cannot judge whether a name is the *right* name, only that a key is not standing in for one. It catches the class this product actually shipped, and no more.
