...
- 02-F45 **Shift attribution is read from the envelope, never from a payload field (August 2026).** `02-F41` rules that attribution is whoever's PIN is in, and `01-F27` puts user identity in the PIN session, which the envelope carries as `actor_user_id`. A `cashier` field duplicated into the `shift.opened` payload would be a **second source for one fact**, and the two can disagree — in an append-only ledger, with no rule for which wins. `packages/sync-client/FOLDS.md`'s `cashier` column is therefore a **projected** column sourced from the envelope's `actor_user_id` field, not a declared payload field. **⚠ NOT YET CONFORMED (August 2026): `packages/sync-client/src/folds/shift-cash.ts` still projects `cashier` from the payload's own `cashier` field.** The shipped renderer emits no such payload — `cash-tab.dom.test.tsx` fails the build on one — so **every live shift row projects `cashier: null`**, and `02-F23`'s own-shifts scoping, though correctly built and mutation-proven, currently narrows nothing. The fold's own oracle carries a PIN reading *"no FR says which one the fold reads"*; **that pin predates this FR and is superseded by it** — this is an unimplemented requirement with a stale pin still pointing at it, not an open question. Owed: the fold reads `actor_user_id`, the pin is retired, and `26 §7`'s fork detector is re-checked, because a per-cashier `prev_shift_id` chain narrows its input. (Until an identity layer exists it projects `null`: `01-F47` admits *devices*, not people, and the PIN session is unbuilt.)
