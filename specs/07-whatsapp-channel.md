...
- 07-F1 Each org gets its **own WABA and its own dedicated number** — never a shared multi-tenant number. Rationale: per-org quality rating and messaging-tier isolation (one org's behavior cannot throttle another), unambiguous inbound routing, and the org owns the asset. Options analyzed:
  - (a) *Org's existing number via Business App + Cloud API coexistence* — Meta supports coexistence for eligible accounts onboarded through a partner supporting Business App number onboarding (verified July 2026): the number and chat history stay usable in the owner's Business App while RestOS uses the Cloud API. Preferred when the org wants its known number; eligibility checked at onboarding.
  ...
  - (c) *Full migration* of the existing number to Cloud API only (the pre-coexistence path — number leaves the phone apps entirely) — fallback when coexistence eligibility fails and the org still insists on the existing number, with consequences accepted in writing.
  - (d) *Shared platform number* across orgs — rejected outright: identity confusion, routing ambiguity, and one org's spam poisons every org's quality rating.
  - **Recommendation: (a) coexistence when the org wants its existing number and is eligible; (b) provisioned number otherwise (and as the speed default); (c) only as documented fallback.** Provisioning and embedded signup live in doc 15 tooling.
...
**Compliance & metering**
- 07-F18 Opt-out — **this module owns the single canonical event family:** "STOP"/"بند" keywords and Meta-level block events are honored immediately, emitting `customer.opted_out` (and explicit re-consent emits `customer.opted_in`); no other module may emit these. The shared suppression read model built from them gates every outbound path including doc 17 broadcasts. Opt-out state is org-scoped and surfaced on the customer file.
