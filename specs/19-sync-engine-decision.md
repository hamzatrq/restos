...
| **Kafka / Kafka Streams** (also Pulsar, Redpanda, NATS JetStream) | **No — out of category** | Asked often, because "append-only event log" sounds like the same problem. It is not: these are **broker-based server-side** logs. There is no embeddable client store to put on an Android/Electron tablet, and a producer/consumer cannot make progress without reaching a broker — so R1 (branch keeps trading with the WAN down) and R3 (device↔device over shop Wi-Fi) are *structurally* impossible, not merely unimplemented. Kafka Streams is JVM, server-side. They solve durable high-throughput **server** fan-out — the half we did **not** need to build: our cloud side is ~700 lines against Postgres and would gain nothing from operating a cluster at 200 branches |
