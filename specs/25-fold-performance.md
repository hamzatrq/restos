...
## 18. Superseded by spec 26

The reframing that supersedes §8/§9/§13 — **deterministic merge semantics rather than a universal total order** — now lives in **`26-merge-semantics.md`**, split out when this document hit the `23-F3` size cap. This document retains the measured defect (§2, §3), the research verdict (§5, §6), the refuted proposals (§11–§16) and the ablation (§17). **`26` is the live position.**
