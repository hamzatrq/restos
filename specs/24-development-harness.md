...
- 24-F26 **A prop is unasserted until an assertion RUNS in each of its states — mounting a value is not exercising it.** *(August 2026, filed after this exact hole appeared THREE times in one component's suite, each time surviving a round that had just closed the previous one.)* `24-F10`'s mutation law finds self-confirming tests; it does not find a state that no assertion was ever pointed at, because a mutant that only misbehaves in an unmounted state has nothing to fail. The three instances, all one-branch mutants passing a suite written to prevent exactly them: (a) **the fixture answered the question** — the item name contained the category slug, so `toContain(category)` was satisfied by the name and dropping the category label entirely passed; (b) **a three-valued prop mounted at two values**, so the unmounted third could be handled by returning nothing at all; (c) **a two-valued prop mounted at both and asserted at one** — every content assertion defaulted the photo away, so an implementation rendering a photographed tile with no price, no name, no category and no sold-out flag passed the whole suite. **(c) is the one to remember: it let `27-F69`'s own filed defect — *"36 tiles, no price on any of them"* — survive the suite written to close it**, on the tiles of a restaurant that had paid to photograph its menu. **The requirement:** for a prop with a small value domain, the suite asserts the *fixture property* — that every value is mounted **and** that the content assertions run under each — rather than trusting a reader to notice. Prefer a **tripwire that reddens when an assertion has no counterpart in a sibling state** over N hand-written duplicates: hand-written twins close the instances you thought of and leave the next one undiscovered, which is how this recurred three times. **And the tripwire itself must be attacked** — `24-F14`'s empty-match rule applies to it, because this repo has already shipped a tripwire that stayed vacuous after its blocker cleared.
