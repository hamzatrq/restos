...
- 10-F3 Every sale on every channel deducts theoretical stock through the recipe chain (menu recipe → raw + prepared; prepared → prep recipe) at the selling location. **Single canonical trigger, channel-agnostic:** a line belongs to deduction iff it is part of a **confirmed** order (`order.confirmed`, or `line_added` after confirm) and was not removed pre-confirm — KOT printing and channel of origin are irrelevant to the invariant. Idempotency key per derived movement row: `(order_line_id, recipe_version)` — one deduction per line, recomputable on recipe correction (10-F4), never doubled by retries or duplicate events. *(Automation law: side-effect of billing.)*
