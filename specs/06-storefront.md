...
- 06-F35 **THE ORDER ID IS MINTED BY THE ORIGIN; THE REQUEST NAMES NO ORDER AT ALL (August 2026, adversarial re-review).** `06-F33` made *"name a price"* unrepresentable and left *"name an order"* free text. `placeOrder` took `order_id` from the public, unauthenticated body, and the origin holds no state and reads none (`06-F30` — it *"appends and pushes, and it never folds a branch stream"*), so it emitted `order.created` plus one line per request **for whatever id the body named**: nothing checked that the id was new, that it belonged to this caller, or that the order was still unconfirmed. Reproduced end to end over real HTTP into a real device fold: an order placed as `web-1`, accepted by a cashier (`order.confirmed`), then a second anonymous request naming `web-1` added 20 × Rs 450 to the confirmed bill — no anomaly raised, the kitchen told to cook it, permanent under `01-F1`. `06-N4` names *"id guessing"* as a probe that must return nothing, and a caller-chosen key is what that probe aims at.
  - **(a) The origin mints it, with the same unguessable source it mints envelope ids from, and RETURNS it.** The cart a customer posts is `{lines: [{line_id, item_id, qty}]}` and carries no order key at all. *Joining an existing order* is therefore **unrepresentable rather than checked** — `06-F33`'s shape and `06-F33`'s reason: a value that can be sent and is then compared is a value a later session trusts, and the comparison is one refactor from being dropped. The minted id is what `order.created` and every `order.line_added` carry, and what the response hands back for `06-F17`'s status page.
  - **(b) `line_id` stays the caller's, and it is not the same hole.** A `line_id` is only ever read inside the order the origin just minted (the fold keys a line cell per order), so two customers cannot collide and a repeated `line_id` is the customer's own cart.
  - **⚠ (c) THE RESIDUAL, NAMED RATHER THAN LEFT SILENT: `cancelOrder` still takes an `order_id` it can neither own nor age-check.** `06-F19` permits a customer cancel *"any time before `order.confirmed`"* and this origin can check **neither** half: it holds no branch slice (`06-F30`), so it cannot know whether the branch has confirmed, and there is no customer session, so it cannot know whose order it is. What (a) closes is the **enumeration** half — after it an order id is an unguessable value rather than `web-1`, so a stranger cannot name one. What remains is (i) a caller who *holds* an id cancelling after confirmation, and (ii) junk `order.cancelled` rows for ids that were never issued, which reach no projection at all (`26 §3`'s sidecar keys on the payload's `order_id`). The two pieces that close it are **`06-F12`'s signed customer session** (ownership) and **`§5`'s order-status read model** (the confirmation state) — both belong to the customer surface `apps/storefront` and neither is invented here. Until they land, the door is open by this FR's decision rather than by omission, and the code says so at the procedure.
