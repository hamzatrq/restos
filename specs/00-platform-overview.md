...
| 04 | `04-waiter-app.md` | Waiter handheld ~~(T3)~~ | App | ~~Android incl. BYOD~~ **any phone, in a browser (R94)** | ~~4~~ **LAUNCH (R86)** |
...
| 10 | `10-inventory-supply.md` | Inventory, recipes, purchasing, wastage, counts, variance, prep planning, forecasting | Service + UI | Cloud + back office + mobile flows | ~~3~~ **LAUNCH (R86)** |
...
| 17 | `17-marketing-loyalty.md` | Broadcasts, promos, loyalty, campaign lift | App + service | Cloud + back office | ~~4~~ **customer file + account loyalty + bearer card: LAUNCH (R86); broadcasts stay 4** |
...
⚠ **WAVES NO LONGER SET THE LAUNCH BOUNDARY (R86/R97, 2026-08-27).** Six surfaces must be complete before restaurant #1 trades — **counter (02), kitchen (03), back office (14), inventory (10), CRM+loyalty (17), waiter pad (04)** — and three of those sat in waves 3 and 4. The wave column now records *dependency order*, which is still true and still useful, and the **LAUNCH** marks record *what ships first*, which is a different question. Where the two disagree, R86 wins. Waves are the dependency order from the concept doc §8: 0 Foundation → 1 Service → 2 Commerce+Delivery → 3 Supply+People → 4 Intelligence+Scale. A module's wave is when its first production slice ships to a dev-pilot restaurant; most modules keep growing afterward.
...
1. **Offline-first — RESCOPED 2026-08-27 by R82/R83 from a universal law to a PER-SURFACE promise.** ~~every in-branch function works with WAN down, indefinitely; branch LAN coordination keeps working~~ — *branch LAN coordination is deleted (R82): two devices in one branch cannot be assumed to share a network.* **The till sells indefinitely with the WAN down and `01-F17` is untouched — a sale is never blocked.** The counter↔kitchen link is best-effort: when both are connected they work together, and when they are not the counter raises an alert and **falls back to paper** (R84). Waiter pads, back office and cloud-only surfaces (storefront, owner app) are cloud-first with an optimistic local buffer and degrade honestly. Cloud-originated orders queue for the branch and enter the moment connectivity returns — the storefront tells the customer the truth about confirmation state.
...
3. **Performance targets** (reference hardware): order line add → UI feedback < 100 ms; confirm → KOT printing starts < 2 s; POS cold start < 6 s; ~~LAN event propagation (device → device) < 1 s p95~~ **(retired with the mesh, R82)**; sync catch-up after 8h offline with ~500 orders < 60 s on 4G; owner dashboard cached load < 2 s.
4. **Security:** TLS everywhere; per-device registration tokens, revocable; PINs Argon2id-hashed, lockout on repeated failure; server-side role authorization (never trust client role claims); org data isolation absolute (customer phone numbers never cross orgs); audit log immutable, hash-chained per device. ⚠ **Amended August 2026 — *TLS everywhere* now states its ENFORCEMENT POINT and its one CARVE-OUT, because it had neither and the device→cloud leg had therefore never met it (`01-F72` (c) says exactly that, in this corpus: *“a law this leg has never met”*).** That leg is **`wss://` only**. It is a load-bearing premise and not a hardening preference: `01-F73` (c) and `01-F80` (f) both reason from *“the cloud leg is TLS to a known endpoint”*, and `01-F80` (f) serves one of exactly **two** unauthenticated writes on that reasoning — while a cleartext dial carries `01-F28`'s Argon2id hashes, `01-F47`'s device token, `01-F81`'s `device_roster` (which decides who may write to a branch ledger) and every event. The asymmetry it corrects ran backwards from the risk: `01-F72` gives the **branch LAN**, on a restaurant's own private network, mutual TLS with a certificate pin, while the leg crossing the public internet took whatever string an environment handed it. **(i) The one carve-out is a LOOPBACK dial.** `ws://` is permitted when, and only when, the host is exactly `127.0.0.1`, `localhost` or `[::1]` — a link that never leaves the machine, and the form every runbook, README and four-process demonstration in this repository uses. It is a **host** test on a PARSED URL and never a substring test, and that distinction is the whole of it: `ws://127.0.0.1.evil.com` and `ws://127.0.0.1@evil.com` both read as loopback to a human and to an `includes()`, and neither is. Userinfo in a cloud URL is refused outright for the same reason — a device authenticates with `01-F47`'s token, so `user@host` has no function here beyond making the host it dials unreadable to whoever is checking it. **(ii) The enforcement point is BOOT, on the boot line, naming the configuration key.** A device that cannot dial securely does not start, returns non-zero (`01-F67`) and says which value is wrong; a lazily-resolved refusal is a till that trades all morning and discovers it at the first sync. **This is not an `01-F17` exception**: an *unset* key is still *offline*, which stays legal and normal (`00 §5.1`), and only a key SET to a cleartext non-loopback endpoint refuses. Enforcing it a second time where the socket is actually dialled is required rather than redundant — one reading, so a host added later cannot dial cleartext by forgetting the boot check. **(iii) Nothing in this repository terminates TLS**, and that is stated here so `wss://` is not read as already satisfied: `services/sync-gateway` listens plain on `0.0.0.0`, so this clause obliges the *deployment* to put a real certificate in front of it. A self-signed one is refused by the client's default verification, which is the honest outcome — no cloud rather than an insecure one — and no code may disable that verification to make a deployment convenient.
...
**(f) ~~The plane does not exist yet —~~ its before-state, measured August 2026, so the migration is a change and not a discovery.** ⚠ **R63's force is this paragraph, not the threshold.** ⚠ **THE PLANE IS BUILT (August 2026, `01-F87`), AND THE THREE BULLETS BELOW ARE STRUCK IN PLACE RATHER THAN DELETED** — this paragraph exists to be the before-state a migration is measured against, so deleting it would delete the measurement; what is corrected is the PRESENT TENSE, because a reader who takes a struck sentence as current is exactly the failure this section warns about elsewhere. **The two bullets that are NOT struck are still true**: `PAID_OUT_APPROVAL_THRESHOLD_PAISA` is still pinned in `apps/pos-electron/src/main/authorize.ts` and the environment keys are still read there — `01-F87` says in terms that *"none of the eight environment stopgaps is retired by this FR … retiring one is a per-key migration in its owning app, which this transport makes possible rather than performs"*, so the transport landing and a key migrating are two acts and only the first has happened.

- ~~**`config.changed` has no payload schema.** The type is in 01 §4's catalog and org-scoped under 01-F62, and `packages/domain`'s registry carries no schema for it, so 01-F4 makes emitting one a runtime error: **a layer-2 change is today unauditable, not merely unbuilt** — and 16-F1 and 16-F4 both record a posture or rule-pack change as exactly this event. Owed to doc 01.~~ **CLOSED by `01-F87` (a)** — the payload is `key` · `layer` · `version` · `before` · `after`, the key space is OPEN in the ledger with the typo check at the writer, and `layer` is a closed enum of this section's own numerals because the type spans layers (15-F25 routes an org's suspension through it).
- ~~**There is no path to a device.** 01-F75's reference-data resource set is **closed** — `catalog`, `staff`, `device_roster` (01-F81) — and configuration is not a member. Whether a fourth member is the answer, or whether layer-2 values ride an existing artifact, is doc 01's call and is owed there; this section states only that no path exists today.~~ **CLOSED by `01-F87`**: `config` is 01-F75's FOURTH member, org-scoped with `branch_id: null`, on the existing frame triple with ZERO new message kinds. A device holds it, resolves each key with (e)'s source, and 01-F87 (b) rules the two dispositions — an unknown key is ignored, a malformed KNOWN key refuses the whole artifact.
- ~~**There is no permission action to gate the write.** The matrix carries **27** actions (comment-blind count, August 2026) and none is about configuration, while `services/api` refuses at boot to host an ungated procedure — so the back-office write **cannot be built or booted**.~~ **CLOSED by `14-F43`** — config.manage, **owner-only**, and the two clauses below survive the closure verbatim because they are what pinned the cells. That is the shape 14-F30, 14-F39 and 02-F46 each closed with an FR-decided action; this one was owed to doc 14 and is paid. ⚠ R63's words are *"the owner or ops lead"* and `ROLES` is `cashier · branch_manager · storekeeper · owner`: **there is no ops lead**, and inventing one here would answer doc 14 §9's open question about a manager's back-office slice by accident. Not decided here.
...
- ⚠ **THE LIST ABOVE IS STALE RATHER THAN WRONG, AND IS CORRECTED HERE RATHER THAN REWRITTEN.** Three keys have joined it since it was measured: `RESTOS_TAX_POSTURE` and `RESTOS_TAX_RATE_BPS` (**one** setting — `16-F27`'s posture cell — on **two** keys, `apps/pos-electron/src/main/tax-posture.ts`), and `RESTOS_CHARGE_ROUNDING_PAISA` (`02-F63`'s charge granularity, same file). All three are named stopgaps in code, as the next bullet requires, and all three die with `01-F87`'s carrier. **Read the list off the code rather than off this paragraph** — a count in prose beside a `grep`-able fact is the staleness this section exists to prevent, and it has now drifted once.
