...
Mutation gates (§2.14); assertion-free-test lint; property tests mandatory for algorithmic code; coverage gates only where they mean something (§2.2). **Separation rule: the AI session that implements a change never writes its own acceptance tests** — tests come from a separate session/agent working from the spec, ~~or from the senior~~ (*there is no senior — §4.4*). (Unit tests alongside implementation are fine; the *acceptance* layer is independent.)

- **Amended August 2026 (founder ruling R66, `plans/saas-pivot/plan-of-record.md` §0): the separation rule is TIERED BY PATH, not universal.** It binds in full where a defect is **permanent and unfixable by a later release** — the protected paths of §4.4 (`packages/domain`, `packages/sync-client`, `packages/sync-protocol`, `packages/escpos`, tax, auth), and any change to money arithmetic, the ledger or the wire. Everywhere else the implementing session may write its own acceptance tests **provided it mutation-proves them** (§2.14): break what each assertion claims to own and confirm that assertion fails.
  - **Why the line falls there and not elsewhere.** The rule exists because an author's blind spot survives into their own tests — five suites in one round were each built correctly and never aimed at the case that mattered. That cost is real but recoverable on an ordinary path: a wrong projection ships, is found, and is fixed. On an **append-only** ledger it is not recoverable — `01-F1` makes a wrong money event permanent, correctable only by a compensating event, for ever. So the tier tracks *reversibility*, which is the property that actually differs, rather than *risk*, which is a judgement every session makes in its own favour.
  - ⚠ **What this costs, stated rather than discovered:** on untiered paths nothing but the author's own mutation discipline stands between a vacuous suite and a green build, and mutation is exactly what an author under time pressure drops first. The ruling was taken with the project a month past its v0 date and no pilot running; **it is the right trade at that state and not a permanent law.** Revisit when a pilot is live, because a defect in front of a paying restaurant is a different cost from one in front of nobody.
  - **What it does NOT change.** §4.4's adversarial review in a separate context is untouched and still binds every protected-path change. §2.14's mutation gates are untouched. `24-F14`'s empty-match protection is untouched. And the round-3 requirement stands wherever tests are written at all: *reading a suite does not detect a vacuous assertion — only mutation does.*

### 4.4 Review lanes
Every PR: (1) adversarial AI review in a fresh context — prompted to find why the change is wrong, not to summarize it; ~~(2) human senior review. CODEOWNERS enforces senior sign-off on protected paths~~ — **superseded August 2026 (founder ruling): there is no senior. Leg (1) becomes the binding gate on protected paths, run as an adversarial review in a SEPARATE AGENT CONTEXT that has not seen the implementing session's reasoning.** Protected paths are unchanged: `packages/domain`, `packages/sync-client`, `packages/sync-protocol`, `packages/escpos` encoder, tax module, auth. Small-PR rule: one module, one concern; a PR touching > 2 packages needs a stated reason in its description.

**The three conditions that make it a gate rather than a formality**, because a review lane that always says SHIP is the vacuous test of §4.3 wearing a governance hat:
- **Fresh context is the mechanism, not a nicety.** The reviewer is told the FR IDs and the diff and is *not* told why the author believed it correct — an author's rationale is exactly the frame that produced the blind spot, and a reviewer handed it reviews the rationale instead of the code.
- **The verdict must be able to be REJECT, and a review that has never rejected anything is evidence about the review.** The reviewer is prompted to find the failure, and "no finding" is a claim it must support by naming what it checked and what it could not.
- **A review is not a substitute for a mutant.** §4.3's mutation requirement and this lane are independent: reading a diff does not detect a vacuous assertion, which is the standing round-3 finding. Where a change carries a security property, the mutation evidence is produced by the *author* out-of-tree and the reviewer's job is to attack the evidence, not to re-run it.

**What is knowingly given up, recorded so nobody reads this as an equivalent swap.** A human senior carries accountability, memory across PRs, and a stake in the consequence; an agent reviewer has none of the three, cannot be trusted to notice that the same mistake has now been made four times, and shares a training distribution with the author — so the failure modes correlate in a way two humans' do not. This is a smaller gate than the one it replaces. It is chosen because an unreviewed protected path is smaller still, and because leg (1) was always specified and is the half that actually reads the code.
