...
import { resolveCampaignArtifact, stampCampaignVersion } from "../campaigns";
import {
  createCausedAppend,
  createGateway,
...
/**
 * The same source, COMMENT-BLIND. Copied from `loyalty-seam.test.ts`'s `codeOf` rather than
 * imported, because that file belongs to another task and `24 §3b` forbids the drive-by; the
 * duplication is named so a later consolidation knows there are two.
 *
 * ⚠ **F0 IS WHY IT IS HERE, AND THE REASON IS A MEASUREMENT, NOT A STYLE PREFERENCE.** That sweep
 * discovers the write road by data flow — `X({ … writes: … })` — with a regex whose middle is
 * `[^})]*`, so a single `)` between a factory's name and its `writes:` key hides the whole
 * construction. The CRM merge wrote one: the `17-F27` (c) comment it added inside
 * `authorizeEscalation({ … })` contains the literal `(c)`, and `authorizeEscalation` **silently
 * left the sweep** — measured on three revisions, present at the waiter branch's own tip and
 * absent on the merge. `24-F14`'s empty-match guard cannot see that, because the count stayed
 * non-zero. The audit's own header says an audit that silently omits a wrapper is the same defect
 * wearing the audit's name; a comment is not a data flow, so it is removed before the sweep runs
 * (`AGENTS.md` `L5`: strip comments, or say plainly that you counted mentions).
 *
 * Strings are deliberately NOT stripped — the sweep's subject is code, and no assertion here
...
/**
 * `01-F23`'s local dialling form, for F7's counter-road control. Eleven unbroken digits, because
 * `normalizeDialledPhone` strips nothing (`customer-phone.ts` argues that at length) — a shape the
 * counter's keypad cannot produce would make the control fail for the wrong reason.
 */
...
// the audit's name.
//
// The six guards a renderer append passes, in order, and where each is answered:
//
//   1. the matrix (`authorizeWrites` → `verdictFor`)        F1 (driven, both roads)
//   2. `stampCampaignVersion`   — `discount.recorded` only  F2 (pass-through) + F3 (unreachable)
//   3. `voidExitsLine`          — `void.recorded` only      F2 + F3
//   4. `refuseZeroTender`       — `payment.recorded` only   F2 + F3
//   5. `refuseDoubleSettlement` — `payment.recorded` only   F2 + F3
//   6. `checkedAppend` — schema parse · `02-F49` · envelope F4 (schema) · §A/§B · F5 (envelope)
//
// ⚠ **ROW 2 ARRIVED FROM ANOTHER MODULE AND IS THE AUDIT'S FIRST REAL CATCH.** `17-F27` (c) put
// `stampCampaignVersion` on the counter's road in a branch that never touched this file, and F0
// went red on the merge — which is the whole point of a tripwire that sweeps the SHIPPED host
// rather than a list someone maintains. Its verdict is the same as rows 3–5: the pad cannot
// express `discount.recorded`, so the stamp is proved UNREACHABLE rather than added as a row and
// forgotten. `04 §1` puts *"void/comp/discount approval"* off the handheld by design and
// `17-F27` (b) makes the CASHIER the one who cites, at `02-F61`'s correction surface — a counter
// screen. The exclusion is asserted in F3 and not merely recorded here.
//
// ⚠ **AND ONE THING THAT IS NOT A GUARD AT ALL, decided in F7 rather than left to the types.**
// `02-F64` added `linkCustomer` to `RendererWrites` in the same merge, so the counter has a write
// channel the pad has no answer for. The answer is that the pad has no customer surface: `04 §1`
// (*"customer-file browsing … not on it, by design"*), `04 §7` (*"deliberately not configurable:
// widening a … slice to include payment/cash/customer data (never)"*), and `04-F17`, whose
// customer-file exclusion `04-F21` marks NOT NEEDED — not because it was relaxed, but because a
// terminal holds nothing, so the privacy property arrives structurally. Reading that mark as
// permission would invert it. `02-F64` authorizes the link under `customer.record`, a DIFFERENT
// action from the `order.create` that `04-F23` rules is the pad's whole set — so building it here
// would widen the surface `04-F22`'s blast-radius claim rests on, and that is a doc-04 spec act.
// F7 drives the exclusion on both roads over one store.
// ─────────────────────────────────────────────────────────────────────────────────────────────
...
describe("§F 04-F27 (a) — every guard on the counter's road, audited against the pad's", () => {
  ...
  const counterChain = (r: Rig) =>
    authorizeWrites({
      writes: stampCampaignVersion({
        ...
        // This device holds no campaigns, which is the ordinary state of a till the `17-F22`
        // resource has never reached and is `EMPTY` rather than `malformed`. Nothing in §F asks a
        // campaign question — F3 proves the pad cannot ask one at all — so an artifact with rows
        // in it would be a fixture no assertion here reads (`17-F22`'s own arms are
        // `loyalty-seam.test.ts` §G's).
        artifact: () => resolveCampaignArtifact({}),
      }),
      store: r.store,
      session: () => ({ user_id: CASHIER, display_name: "Ayesha" }),
  ...
  it("F0 — everything the host puts on the renderer's write road is a row in this audit", () => {
    /**
     * The tripwire that makes the rest of §F an audit rather than a snapshot. A new thing
     * appearing between the renderer and the ledger is a guard nobody has asked the pad about —
     * `02-F49`'s defect one layer up — so it reddens here and the author has to add a row. It has
     * now caught one from another module: `17-F27` (c)'s campaign stamp, merged from a branch that
     * never opened this file.
     *
     ...
     *
     * ⚠ **AND IT IS SWEPT COMMENT-BLIND, because the second thing the CRM merge did to this
     * tripwire was to make it go PARTIALLY blind, which is worse than the red it also produced.**
     * `stampCampaignVersion` arriving is the catch everyone sees; `authorizeEscalation`
     * *departing* is the one nobody would have. Its `writes:` key sits behind a comment the merge
     * extended with the literal `(c)`, and the middle of the second pattern below is `[^})]*` —
     * so one parenthesis of prose removed a real consumer from a sweep whose count stayed
     * non-zero and whose `24-F14` guard therefore stayed happy. See `codeOf`.
     *
     * ⚠ **TWO THINGS THIS ROW DOES NOT OWN, measured as mutants rather than reasoned, and named
     * so the next reader does not credit it with them (`AGENTS.md` `L11`).**
     * (i) It sees a CONSTRUCTION, not a reachability. Unwiring `stampCampaignVersion` from the
     * two `authorizeWrites`/`authorizeEscalation` call sites while leaving it declared keeps this
     * row green — the guard would be built, correct and on no road, which is `L8` exactly. Four
     * assertions in `loyalty-seam.test.ts` kill that mutant and this one does not.
     * (ii) It is a LIST, and a list can be rewritten into a snapshot of itself. Replacing the
     * expected set with `[...new Set(consumers)].sort()` kills **nothing in this package** — no
     * suite can catch a test that agrees with whatever it finds. What survives that is the rest
     * of §F: under the tautology *plus* a real divergence (`discount.recorded` added to
     * `TERMINAL_EVENT_TYPES`) the suite still goes red, in **F3**, because F3 drives the decision
     * instead of listing it. That is why the campaign row's verdict lives there and not here.
     */
    const mainCode = codeOf(mainSrc);
    // `24-F14`, per half: the strip must have removed something and must have kept the code. The
    // second anchor is the construction that went missing, named rather than covered by a length.
    expect(mainCode.length).toBeLessThan(mainSrc.length);
    expect(mainCode.length).toBeGreaterThan(20_000);
    expect(
      mainCode,
      "the comment strip ate the escalation construction — the sweep below is measuring a hole",
    ).toContain("const escalation = authorizeEscalation({");
    const consumers = [...mainCode.matchAll(/(\w+)\(\{\s*writes:|(\w+)\(\{[^})]*\bwrites:/g)]
    ...
    expect(
      ...
      [
        // Guards 3–5 of the audit: rows F2 and F3.
        "refuseDoubleSettlement",
        "refuseZeroTender",
        "voidExitsLine",
        // Guard 2, and the one this tripwire actually caught (`17-F27` (c), merged from another
        // branch). It is a ROW because the audit has decided it, not because it appeared: the pad
        // cannot express `discount.recorded` at all, which F3 drives, and F2 proves the stamp
        // decides nothing about the four types it CAN express. Its neighbouring case is named at
        // `authorize.ts`'s terminal limits — the pad's road would not stamp a citation either, and
        // F3 is what makes that a red test the day the surface widens.
  ...
  it("F2 — the four money/void/campaign wrappers are PASS-THROUGHS for every type the pad can express", async () => {
    /**
     * The audit's real question for guards 2–5: not *"does the pad reach them"* (it does not) but
     ...
     *
     * ⚠ **`stampCampaignVersion` IS WHY "UNCHANGED" IS NOW MEASURED AND NOT JUST "DID NOT THROW".**
     * The other three refuse by throwing, so `not.toThrow()` is the whole question for them. This
     * one refuses nothing and never throws — what it does is EDIT A PAYLOAD (`17-F27` (c)) — so
     * the inherited assertion is vacuous against it by construction, which is `L10`'s guard built
     * correctly and aimed one case away. What is asserted is that every request arrives at the
     * ledger byte-for-byte as the pad's road would have produced it.
     */
    const r = await rig();
    const handle = await signedIn(r);
    const { order, line } = await cookingOrder(r, handle);
    const seen: { type: string; payload: unknown; refs: unknown }[] = [];
    const spy = {
      append: (req: unknown) => {
        seen.push(req as { type: string; payload: unknown; refs: unknown });
        return { id: "spy" };
      },
      addLine: () => ({ id: "spy" }),
      toggleAvailability: () => ({ id: "spy" }),
      recordCustomer: () => ({ id: "spy" }),
      // `02-F64`'s link. Here because `RendererWrites` requires it and for no other reason — F7
      // is where the pad's relationship with it is decided, and it is an exclusion, not a row.
      linkCustomer: () => ({ id: "spy" }),
    };
    // The shipped nesting, outermost first, exactly as `main/index.ts` composes it below the
    // matrix: campaign stamp → void exit → amount → duplicate → ledger.
    const wrapped = stampCampaignVersion({
      writes: voidExitsLine({
        writes: refuseZeroTender({
    ...
    const sent: { type: string; payload: unknown; refs: unknown }[] = [];
        ...
        type === "order.created"
          ...
          : type === "order.line_added"
            ...
            : type === "order.line_removed"
              ...
              : { order_id: order };
      const req = { type, payload, refs: [] };
      sent.push(req);
      expect(
        () => wrapped.append(req),
        `a money/void/campaign wrapper decided something about ${type} — it is a guard the pad does not pass`,
      ).not.toThrow();
    }
    expect(
      seen.map((s) => s.type),
      "a wrapper swallowed a terminal event type",
    ).toEqual([...TERMINAL_EVENT_TYPES]);
    expect(
      seen,
      "a wrapper REWROTE a request the pad produced — the pad's road does not apply it, so the two roads would disagree about what was appended",
    ).toEqual(sent);

    /**
     * THE CONTROL, and without it every assertion above proves nothing about the stamp: an
     * implementation that dropped `stampCampaignVersion` from this chain — or replaced it with the
     * identity function — satisfies all of them (measured: 0 failures). So the ONE type it does
     * bear on is driven through the SAME object, and must come out changed.
     *
     * The drop arm rather than the stamp arm, because this device deliberately holds no artifact:
     * a citation naming a campaign it cannot state is dropped rather than recorded, which is
     * `17-F27` (c)'s answer to `17-F25`'s *"under what rule?"*. `loyalty-seam.test.ts` §G owns the
     * rest of the behaviour; what is being established here is that the wrapper in this chain is
     * the real one.
     */
    wrapped.append({
      ...
      payload: {
        ...
        adjustment_attempt_id: "att-f2-control",
        campaign_id: "camp-this-device-does-not-hold",
        campaign_version: 999,
      },
      refs: [],
    });
    const control = seen.at(-1)?.payload as Record<string, unknown>;
    expect(
      control,
      "the campaign stamp in this chain is a no-op, so every pass-through assertion above is vacuous",
    ).not.toHaveProperty("campaign_id");
    expect(control).not.toHaveProperty("campaign_version");
    // …and `01-F17`: the discount itself still landed. Nothing here blocks a sale.
    expect(control?.amount_paisa).toBe(100);
  });

  it("F3 — and the three types they DO bear on cannot be expressed by the pad at all", async () => {
    // Guards 2–5 are not SKIPPED on the pad's road; they are unreachable, because `04-F23`'s gate
    ...
    for (const kind of [
      ...
      "void.recorded",
      "settle",
      // `17-F27` (c)'s subject type, on exactly the same terms as the four above.
      "discount",
      "discount.recorded",
    ]) {
      expect(
    ...
    /**
     * ⚠ **THE ROW THAT DECIDES `stampCampaignVersion`, AND IT CARRIES A SECOND DEBT.**
     *
     * Refused at gate 1, so `verdictFor`'s discount branch is never entered and the
     * `campaignCitation: undefined` this gate supplies is never read — which is what makes that
     * `undefined` a decision rather than an omission (`authorize.ts` states the reasoning; this is
     * the assertion that holds it).
     *
     * **What is NOT closed, named here because it is one keystroke away (`AGENTS.md` `L11`).**
     * The pad's road is `appendAs` → `checkedAppend`; it is not built over the renderer's `writes`
     * chain, so it would not stamp a `campaign_version` either. Today that is unobservable — this
     * refusal is why. The day someone adds `discount.recorded` to `TERMINAL_EVENT_TYPES` (a doc-04
     * spec act: `04 §1` puts discount approval off the handheld, and `17-F27` (b) makes the
     * CASHIER the one who cites at `02-F61`'s counter surface), **this assertion goes red first**,
     * and the stamp has to be decided in the same change rather than discovered afterwards by an
     * unanswerable `17-F25` on a permanent event (`01-F1`).
     */
    expect(
      gate(WAITER, "discount.recorded"),
      "the pad can now express a discount — decide `17-F27` (c)'s stamp for this road before this passes",
    ).toEqual({ ok: false, refusal: "not_a_terminal_act" });
  });
  ...
  it("F7 — 02-F64: the counter can attach a customer to an order and the pad has no such act", async () => {
    /**
     * **THE AUDIT'S OTHER VERDICT, AND IT IS AN EXCLUSION RATHER THAN A GUARD.** `02-F64` put
     * `linkCustomer` on `RendererWrites` in the same merge that put `stampCampaignVersion` on the
     * road above, so the counter gained a fifth write channel with no terminal twin. §F asks the
     * same question of it that it asks of a guard — *does the pad reach this, and if not, is that
     * DECIDED* — because an omission and a decision look identical in a diff and only one of them
     * survives the next reader.
     *
     * **The decision: the pad does not link customers, and building it here would be a doc-04
     * spec act.** `04 §1` (*"customer-file browsing … not on it, by design"*) and `04 §7`
     * (*"deliberately not configurable: widening a … slice to include payment/cash/customer data
     * (never)"*) both say so, and `04-F17` — whose customer-file exclusion `04-F21` marks NOT
     * NEEDED because a terminal HOLDS nothing rather than because the exclusion was relaxed —
     * must not be read the other way round. `02-F64` authorizes the link under
     * **`customer.record`** — a different action from the `order.create` that `04-F23` rules is
     * the pad's entire set. Adding it would take the pad from one action to two, which is the
     * claim `04-F22`'s blast radius rests on.
     *
     * **It is not an `01-F17` block.** `02-F64` says it itself: an order with no link is an
     * ordinary order that no loyalty counter, phone search or khata can reach — the state every
     * order in this product was in before that FR. The waiter's order rings, confirms and reaches
     * the kitchen; the counter attaches the caller at settlement, where `02-F14`'s khata and
     * `17-F17`'s reward already live.
     */
    const r = await rig();
    const handle = await signedIn(r);
    const { order } = await cookingOrder(r, handle);

    // THE CONTROL, in the same test and over the same store: the counter's road really does have
    // this act, for this very person's role. Without it, every refusal below could be a matrix
    // that denies everyone rather than a surface that is narrower than the role (`04-F23`).
    expect(() => counterChain(r).linkCustomer({ order_id: order, dialled: DIALLED })).not.toThrow();
    expect(r.events()).toContain("order.customer_linked");

    // ── (a) The pad cannot express it, and gate 1 answers before the roster is even consulted. ──
    const gate = authorizeTerminal({ store: r.store });
    expect(
      gate(WAITER, "order.customer_linked"),
      "the pad gained a customer act — `02-F64` authorizes it under `customer.record`, which is a second action on a surface `04-F23` closed at one",
    ).toEqual({ ok: false, refusal: "not_a_terminal_act" });
    expect(gate(WAITER, "customer.created")).toEqual({
      ok: false,
      refusal: "not_a_terminal_act",
    });

    // ── (b) …and the intent union has no shape for it, so the refusal arrives before the gate. ──
    const before = r.events().length;
    for (const intent of [
      { kind: "link_customer", order_id: order, dialled: DIALLED },
      { kind: "record_customer", dialled: DIALLED, name: "Sana" },
    ]) {
      const result = r.terminal.act(handle, intent);
      expect(result.ok, `the pad expressed ${intent.kind}`).toBe(false);
      if (result.ok) return;
      // `malformed`, never `not_permitted`: the pad must not render "you may not" for an act that
      // does not exist on this surface at all (`terminal.ts` keeps the two distinct on purpose).
      expect(result.reason).toBe("malformed");
    }
    expect(r.events().length, "an act outside the terminal's surface reached the ledger").toBe(
      before,
    );

    // ── (c) THE STRUCTURAL HALF, and it is the one that owns the real risk. ─────────────────────
    // (a) and (b) bound what the pad may APPEND. `linkCustomer` is not an append — it is a
    // separate channel on the gateway object, so gate 1 would never see it, exactly as gate 1
    // never saw `04-F33`'s settlement consequence. The host hands `createTerminal` the WHOLE
    // gateway as `reads` (`TerminalDeps` narrows it to `menu | openOrders` in the type, and a type
    // is not a runtime gate), so what is measured is which members a full pad flow actually
    // touches. A terminal that reached `reads.linkCustomer` — or `recordCustomer`, or any of the
    // money reads — would pass every assertion above.
    const touched = new Set<string>();
    const probed = new Proxy(r.gateway, {
      get(target, prop, receiver) {
        if (typeof prop === "string") touched.add(prop);
        return Reflect.get(target, prop, receiver);
      },
    }) as Gateway;
    const overProbe = createTerminal({
      verifyWaiter: async () => ({ ok: true }) as never,
      authorize: authorizeTerminal({ store: r.store }),
      appendAs: createVerifiedAppend({ store: r.store }),
      addLineAs: createVerifiedAddLine({ store: r.store, priceOf: () => 6_000 }),
      onAppended: () => {},
      reads: probed,
      store: r.store,
      idle_lock_ms: 10 * 60_000,
      now: () => 1_754_300_000_000,
      newHandle: () => "probe-handle",
    });
    const probeSignIn = await overProbe.signIn(WAITER, PIN);
    expect(probeSignIn.ok).toBe(true);
    if (!probeSignIn.ok) return;
    const probeHandle = probeSignIn.handle;
    // A whole order's worth of pad work, so the sweep is over what the surface really does and
    // not over one call (`24-F14`: an empty flow would make the set below trivially correct).
    const opened = overProbe.act(probeHandle, { kind: "open", table_id: "9" });
    expect(opened.ok, "the probe rig could not open a table").toBe(true);
    if (!opened.ok) return;
    expect(
      overProbe.act(probeHandle, {
        kind: "add_line",
        order_id: opened.order_id,
        item_id: NAAN,
        qty: 1,
    ...
    expect(overProbe.act(probeHandle, { kind: "confirm", order_id: opened.order_id }).ok).toBe(
      true,
    );
    expect(overProbe.view(probeHandle)).not.toBe(null);
    expect(
      [...touched].sort(),
      "the terminal reached a gateway member outside `menu`/`openOrders` — the pad's road is no longer bounded by its own dep type",
    ).toEqual(["menu", "openOrders"]);
  });
});
