 ...
 * vitest** — the same technique and the same reason as `catalog-seam.test.ts` §D, which is this
 * app's established pattern for asserting that the entry point wires something.
 *
 ...
 * is NOT usable as the anchor: it occurs earlier in this file inside a doc comment, and in
 * `apps/pass-kds` it occurs inside `boot()` *above* the module-scope prologue — a first draft of
 * this file used it and produced two false failures, which is the cheap version of the lesson.
 *
...
const BOOT_KICKOFF = "const counterBoot = app.whenReady()";
...
describe("§A — 00 §5.4 (ii): the refusal is at BOOT, before anything opens", () => {
   ...
   * `counterBoot`, inside an IPC handler, in a branch reached only once a socket is wanted — and
  ...
  /**
   * `01-F67`: stderr because it is the stream `ops/startup/restos-counter.bat` captures, and a
   * NON-ZERO exit because that launcher tells a permanent refusal from a crash by the shape of the
   * return. `app.quit()` here would be a till that reported success while refusing to work, and a
...
describe("§B — 00 §5.7: the boot line reports the transport", () => {
  it("prints describeCloudUrl, so a cleartext loopback run says so out loud", () => {
    expect(mainSrc).toMatch(/console\.log\(describeCloudUrl\(/);
    expect(mainSrc).toMatch(
