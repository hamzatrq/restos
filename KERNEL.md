...
**State AT THE WAVE-0 KERNEL MERGE — a snapshot, not the tree you are looking at:** `pnpm verify` green; `pnpm verify:01` = **34 green / 0 red / 13 unmapped of 47 FRs**; full suites green — domain 98, sync-protocol 32, sync-client 264, testing 49, gateway 151 (on real Postgres via Testcontainers). `pnpm test` needs Docker. ⚠ *This line was headed "Current state" and described `pnpm verify` as "(docs-lint + typecheck + lint)" — the same stale composition `AGENTS.md` carried. `verify` gained `tokens:check`, `seams:check` and `layout:check` since, and every suite count here has roughly tripled. **`AGENTS.md`'s repo-state line is the live measurement; this document is the Wave-0 review record and should be read as dated throughout.***
...
> **UPDATE (July 2026): the post-review round is COMPLETE.** Everything the bullet below scheduled has landed — T-01-17 (time), T-01-18 (auth), T-01-19 (compression), T-01-20 (catch-up status), T-01-21 (quarantine key), T-01-22 (fold money), each through the full `24 §3` loop. Suites: domain 131, sync-protocol 62, sync-client 320, testing 49, gateway 195. Four defects invisible to green suites were found by the oracle/adversarial legs and fixed: a projected value depending on envelope-id sort order, an honest origin's quarantine evidence being discarded, a privilege escalation on token renewal, and branch time teleporting onto an untrusted clock at hub handover. Remaining open items are in `plans/wave-0/sec-review-followups.md`; the reviewer-facing text below is kept for context.

- **Founder decisions — ratified (July 2026), and now implemented:** `DEC-TIME-001` accepted and promoted into `01-F43..F46` (branch-consensus time on the hub; absolute stamps provisional-until-reconciled; `device_created_at` demoted to an untrusted hint; Asia/Karachi day boundary) — until **T-01-17** lands, `confirmed_at`/`kot_at`/`age_basis` still stamp from an untrusted clock, so treat those values as unreliable when reviewing. `DEC-AUTH-001`/`DEC-AUTH-002` accepted (`01-F47`/`01-F48`: 90-day token expiry with silent hub-relayed renewal, `iss`/`aud` binding, 30 s fail-closed revocation eviction) — **T-01-18**. `DEC-SYNC-010` (per-connection compression negotiation) — **T-01-19**. `DEC-SYNC-011` (blocked catch-up cursor is observable; stop-and-report, never skip) — **T-01-20**. The four product constants in `packages/domain/src/product-constants.ts` are ratified at their matrix defaults and are no longer open.
