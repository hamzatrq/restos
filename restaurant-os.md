...
- Hardware is a **capability set, not a ladder** (founder ruling, August 2026 — `DEC-HW-003`, `DEC-HW-001`): a branch declares what it actually has — a printer per station, a pass screen, a KDS, waiter handhelds, a cash drawer — and the product adapts to that set. **This is a bring-your-own-hardware product**; restaurants arrive with arbitrary combinations (a printer at the grill and a phone at the cold station; a drawer and no printer; a TV someone had in storage), and a linear rung cannot express any of them. The ladder was the right model for selling hardware packages and the wrong one for a customer who already owns theirs. **T1 Counter** (terminal + printers) · **T2 Counter+Pass** (adds pass screen w/ ready-marking) · **T3 Full mesh** (waiter handhelds, station routing/KDS, manager console) survive as **derived labels** over that set — useful for selling, support and onboarding, and still what `02-F31` detects — but the set is what the code reads. **No code may branch on the tier to decide whether a piece of hardware exists.** Capabilities upgradeable; a smaller set degrades gracefully and never feels crippled, and absence of a peripheral is a **configuration**, never a fault (`03-F51`).
...
- **Recipes (~~vendor-onboarded, never owner data entry~~ **authored by the owner in the back office; the RestOS team uses the same editor on a pilot's behalf — founder decision 2, August 2026; see `specs/10 §1`**):** menu recipes (sold item → ingredients) and prep recipes with yield % ("18 kg raw → 15 kg marinated boti"). **Scope discipline: ~~top 10–20 high-cost ingredients only (~70% of food cost)~~ ⚠ AMENDED by `10-F31` (August 2026) — the top 10–20 discipline binds the COUNT; COSTING covers everything, because a recipe complete under the struck rule still costs only ~70% of a dish and a food-cost figure built on it understates by ~30% permanently. The recurring workload is unchanged; an estimated 20–40 rows move to one-time onboarding. Read `specs/10-F31` before quoting this line** — full-menu costing is where competitors' modules die.
